...
if (!function_exists('registerHelper')) {
    /**
     * Register helper classes and expose them as global helper functions.
     *
     * Each helper class must provide a public handle method and may define:
     *  - $signature (string)           The function name to register.
     *  - $cli / $web (bool)            Runtime guards for CLI or HTTP contexts.
     *  - getSignature()/signature()    Optional accessors returning the function name.
     *
     * @param array<int, class-string>|class-string $helpers
     */
    function registerHelper(array|string $helpers): void
    {
        $helpers = is_array($helpers) ? array_values(array_filter($helpers)) : [$helpers];
        $helpers = array_unique($helpers);
        $registryKey = '__zero_registered_helpers';
        ...
        foreach ($helpers as $helperClass) {
            ...
            if (isset($GLOBALS[$registryKey][$signature])) {
                continue;
            }

            $GLOBALS[$registryKey][$signature] = static function (...$arguments) use ($instance) {
